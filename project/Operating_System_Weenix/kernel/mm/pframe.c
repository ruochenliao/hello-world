...
{
  /*int i;*/
  		pframe_t *page_frame  = pframe_get_resident(o , pagenum);
		if (page_frame == NULL){
			if( pageoutd_needed() ){
				pageoutd_wakeup();
				dbg(DBG_PRINT,"pageoutd is wake up and the page is freed\n");
			}

			page_frame = pframe_alloc (o,pagenum);
			if (page_frame == NULL ){
				dbg(DBG_PRINT, "Page is not allocated with memory\n");
				return -1;
			}
			else{
				pframe_fill( page_frame );
				*result  = page_frame;
				dbg(DBG_PRINT,"new page is returned\n");
				return 0;
			}
		}

		else{
		  while ( pframe_is_busy(page_frame) == 1)
			   {
			     sched_sleep_on(&page_frame->pf_waitq);
			  }
  				dbg(DBG_PRINT,"existing page is returned with **result\n");
				*result = page_frame;
				return 0;
		}
...
pframe_pin(pframe_t *pf)
{
  KASSERT(!pframe_is_free(pf));
	dbg(DBG_PRINT,"GRADING3A 1.a: (precondition) KASSERT(!pframe_is_free(pf))\n");
	KASSERT(pf->pf_pincount >= 0);
	dbg(DBG_PRINT,"GRADING3A 1.a: (precondition) KASSERT(pf->pf_pincount >= 0);\n");
...
{
    dbg(DBG_PRINT,"(precondition) KASSERT(!pframe_is_free(pf))");
  dbg(DBG_PRINT,"(precondition) KASSERT(pf->pf_pincount > 0)");
    pf -> pf_pincount --;
	if( pf -> pf_pincount == 0){
		...
		npinned --;
		nallocated ++;
		dbg(DBG_PRINT,"pframe_unpin is called successful \n");
	}
