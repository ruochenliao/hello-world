...
sys_read(read_args_t *arg)
{
  ...
  err=copy_from_user(&arg2, arg, sizeof(arg2));
  if(err<0)
    {
      curthr->kt_errno= -err;
      return -1;
  ...
  /*Temporary buffer? or the buffer of arg2??*/
  temp_buffer=page_alloc();
  ...
  bytes_read=do_read(arg2.fd, temp_buffer, arg2.nbytes);
  if(bytes_read<0)
    {
  ...
  err=copy_to_user(arg2.buf,temp_buffer,bytes_read);
  if(err<0)
    {
      curthr->kt_errno=-err;
      return -1;
...
write_args_t arg2;
 int err,bytes_read;
  ...
  if(temp_buffer==NULL)
    {
      curthr->kt_errno=-1;/*Not sure what error*/
      return -1;
    }
  
  err=copy_from_user(temp_buffer,arg2.buf,arg2.nbytes);
  if(err<0)
    {
      curthr->kt_errno=-err;
      return -1;
  ...
  bytes_read=do_write(arg2.fd, temp_buffer, arg2.nbytes);
  if(bytes_read<0)
    {
      curthr->kt_errno=-bytes_read;
      return -1;
...
sys_getdents(getdents_args_t *arg)
{
	...
	dbg(DBG_PRINT, "(GRADING3E) sys_getdents()\n");
	int total = 0;

	/*GS: copy the user args into copy_args*/
	getdents_args_t copy_args;
	if( copy_from_user(&copy_args,arg,sizeof(getdents_args_t)) < 0 )
	{
		dbg(DBG_PRINT, "(GRADING3E) sys_getdents() copy_from_user() < 0\n");
	...
	/*GS: count is the number of bytes in the buffer,
	 * not the number of dirents, so divide bytes by sizeof() dirent to
	 * get number of dirents */

	int dirs = copy_args.count / sizeof(dirent);

	int i = 0;
	dbg(DBG_PRINT, "(GRADING3E) sys_getdents() for(i = 0; i < dirs; i++)\n");
	for(i = 0; i < dirs; i++)
	{
		/*GS: fill in dirent_t. Returns number of bytes read into dirent*/
		int retSize = do_getdent(copy_args.fd,&dirent);

		/*GS: get out of the loop, if there are no more bytes */
		if(retSize == 0)
		{
			dbg(DBG_PRINT, "(GRADING3E) sys_getdents() if(retSize == 0)\n");
			break;
		}
		else if(retSize<0)
		  {
		    curthr->kt_errno=-retSize;
		    return -1;
		  }

		/*GS: copy the dirent just filled, into user space of the
		 * current process. The user address comes from the user args
		 * dirp base pointer, which is it is incremented by
		 * i * sizeof(dirent_t) each loop, pointing to each successive dirent*/
		char *userAddr = (char *)copy_args.dirp + i * sizeof(dirent_t);
		int retVal = copy_to_user(userAddr,&dirent,sizeof(dirent_t));

		/*GS: error case*/
		if(retVal < 0)
		{
			dbg(DBG_PRINT, "(GRADING3E) sys_getdents() if(retVal < 0)\n");
			curthr->kt_errno = retVal;
			return -1;
		}

		/*GS: running total of number of bytes returned*/
		total = total + retSize;
