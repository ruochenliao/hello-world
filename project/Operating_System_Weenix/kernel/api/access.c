...
int addr_perm(struct proc *p, const void *vaddr, int perm)
{
	...
	if( vm_area == NULL ){
		dbg(DBG_PRINT,"error: failed to allocate the memory for vmarea \n");
		return 0;
	}
	else {
	    if ((perm & PROT_READ) && !(vm_area->vma_prot & PROT_READ))
	        return 0;
	    if ((perm & PROT_WRITE) && !(vm_area->vma_prot & PROT_WRITE))
	        return 0;
	    if ((perm & PROT_EXEC) && !(vm_area->vma_prot & PROT_EXEC))
	        return 0;
	    dbg(DBG_PRINT,"return from addr_perm successfully \n");
	    return 1;
	}
}
...
{
  /*Alekhya: There should be another way to do it, But im not sure how to do that. Will think about it. This is the simplest method, which should work*/
  uint32_t temp=0;
  int ret;
  while(temp<len)
    {
      ret=addr_perm(p,(void *)((uint32_t)avaddr+temp),perm);
	if(ret==0)
	  return 0;
	temp++;
    }
