...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
	int bytedev;
	KASSERT(file);
	dbg(DBG_PRINT, "(GRADING2A 1.a) (precondition) KASSERT(file)");

	KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));
	dbg(DBG_PRINT, "(GRADING2A 1.a) (precondition) KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)))");	
	
	if ( S_ISCHR( file->vn_mode) ){
		KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->read);
		dbg(DBG_PRINT, "(GRADING2A 1.a) (precondition) if (S_ISCHR(file->vn_mode)) KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->read) ");
		/*Alekhya: The read needs to be called from cd_ops*/
		bytedev = file->vn_cdev->cd_ops->read( file->vn_cdev, offset, buf, count);
		return bytedev;
	}	

/*	if ( S_ISBLK(file->vn_mode) ){        	
		dbg(DBG_PRINT, "(GRADING2A 1.a) vnode.c specical_file_read blocks, return a -ENOTSUP");
		return -ENOTSUP;
	}
*/
	return 0;
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
	...
	int bytedev;
	KASSERT(file);
	dbg(DBG_PRINT, "(GRADING2A 1.b) (precondition) KASSERT(file)");

	KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));
	dbg(DBG_PRINT, "(GRADING2A 1.b) (precondition) KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)))");	
	
	if ( S_ISCHR( file->vn_mode) ){
		KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->write);
		dbg(DBG_PRINT, "(GRADING2A 1.b) (precondition) if (S_ISCHR(file->vn_mode)) KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->write) ");

		
		bytedev = file->vn_cdev->cd_ops->write( file->vn_cdev, offset, buf, count);
		return bytedev;
	}
	return 0;
...
/*
		...
		dbg(DBG_PRINT, "(GRADING2A 1.b) vnode.c specical_file_write blocks, return a -ENOTSUP");
		return -ENOTSUP;
	}
		...
		dbg(DBG_PRINT, "(GRADING2A 1.b) error happens at vnode.c specical_file_write ");
		return -1;
	}	
*/	
...
{
int return_value = file -> vn_cdev -> cd_ops -> mmap( file, vma, ret );
	return return_value;
...
{
	 int return_value;
	 return_value = file -> vn_cdev -> cd_ops -> fillpage( file , offset, pagebuf );
	 return return_value; 
}
...
{
int return_value;
	 return_value = file -> vn_cdev -> cd_ops -> dirtypage( file , offset );
	 return return_value;
...
{
  int return_value;
	 return_value = file -> vn_cdev -> cd_ops -> cleanpage( file , offset,pagebuf );
	 return return_value;
