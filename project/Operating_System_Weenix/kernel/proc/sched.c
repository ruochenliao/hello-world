...
sched_wakeup_on(ktqueue_t *q)
{
    /*Andy*/
        ...
        dbg(DBG_PRINT,"GRADING1A 4.a (thr->kt_state == KT_SLEEP) || (thr->kt_state == KT_SLEEP_CANCELLABLE)");
        thr->kt_state=KT_RUN;
...
sched_cancel(struct kthread *kthr)
{
    /*Andy*/
    kthr->kt_cancelled=1;
    if(kthr->kt_state==KT_SLEEP_CANCELLABLE||kthr->kt_state==KT_EXITED||kthr->kt_state==KT_NO_STATE){
...
sched_switch(void)
{
    ...
    /*sched_sleep_on(&curthr->kt_proc->p_wait);*/
    oldthr=curthr;
...
sched_make_runnable(kthread_t *thr)
{
    ...
    dbg(DBG_PRINT,"Grading 4.b &kt_runq != thr->kt_wchan\n");

    uint8_t oldipl=intr_getipl();
    intr_setipl(IPL_HIGH);
    
    /*transfer the waitqueue of the process to kt_runq*/
        thr->kt_state=KT_RUN;
	    if(thr->kt_proc->p_pid!=PID_IDLE){
            ktqueue_enqueue(&kt_runq,thr);
	    }
	intr_setipl(oldipl);
